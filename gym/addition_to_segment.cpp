...
#define ll long long
using namespace std;
// Get-Content t.txt | .\addition_to_segment.exe
...
ll query(ll node, ll left, ll right, ll x, ll y) {
    ...
    // Partial overlap
    ll middle = (left + right) / 2;
    return query(2 * node + 1, left, middle, x, y) + query(2 * node + 2, middle + 1, right, x, y);
}
...
int main() {
    ...
    for (ll i = 0; i < m; i++) {
        ...
        if (op == 1) { // Update operation: set element at index l to value r
            ll l, r, x;
            cin >> l >> r >> x;
            update(0, 0, n - 1, l, r - 1, x); // Update difference
